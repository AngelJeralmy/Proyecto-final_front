...
const createUsuario = async (nombre, correo, password) => {
    try {
        console.log("En create Usuario: ", nombre, correo, password);
        
        // Encriptar la contraseña
        const passwordEncriptada = bcrypt.hashSync(password, 10); // 10 es el número de rondas de sal
        ...
        const { rows } = await pool.query(consulta, values);
        console.log("Filas retornadas: ", rows);

        // Retornar el primer resultado (el usuario creado)
        return rows[0];
    } catch (error) {
        console.error("Error al crear el usuario: ", error);
        throw error;  // Re-lanzar el error para manejarlo en el lugar de la llamada
    }
};
